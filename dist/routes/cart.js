...
function setCartId(req, cartId) {
    const session = req.session;
    session.cartId = cartId;
}
function clearCartId(req) {
    const session = req.session;
...
// BC Cart helpers
// ---------------------------------------------------------------------------
async function fetchRedirectUrls(cartId) {
    const res = await bigcommerce_1.default.post(`/v3/carts/${cartId}/redirect_urls`);
 ...
 */
async function createCart(productId, quantity, variantId) {
    const lineItem = { product_id: productId, quantity };
    if (variantId)
        lineItem.variant_id = variantId;
    const res = await bigcommerce_1.default.post('/v3/carts', {
        line_items: [lineItem],
    });
    return res.data.data;
}
 ...
 */
async function appendCartItem(cartId, productId, quantity, variantId) {
    const lineItem = { product_id: productId, quantity };
    if (variantId)
        lineItem.variant_id = variantId;
    const res = await bigcommerce_1.default.post(`/v3/carts/${cartId}/items`, {
        line_items: [lineItem],
    });
    return res.data.data;
}
...
router.post('/cart/items', async (req, res) => {
    ...
    try {
        ...
        if (existingCartId) {
            try {
                cart = await appendCartItem(existingCartId, productId, quantity, variantId);
            }
            catch (err) {
                // Cart expired or deleted on BC — create a fresh one
                if (err.response?.status === 404) {
                    logger_1.default.warn(`cart ${existingCartId}: not found on BC, creating new cart`);
                    clearCartId(req);
                    cart = await createCart(productId, quantity, variantId);
                }
                else {
                    throw err;
                }
        ...
        logger_1.default.error(`cart add item failed (productId=${productId}): ${err.message}`);
...
router.get('/cart', async (req, res) => {
    ...
    try {
        const cartRes = await bigcommerce_1.default.get(`/v3/carts/${cartId}`, {
        ...
        logger_1.default.error(`cart fetch failed (cartId=${cartId}): ${err.message}`);
...
router.delete('/cart/items/:itemId', async (req, res) => {
    ...
    try {
        await bigcommerce_1.default.delete(`/v3/carts/${cartId}/items/${itemId}`);
        ...
        logger_1.default.error(`cart remove item failed (cartId=${cartId}, itemId=${itemId}): ${err.message}`);
...
router.delete('/cart', async (req, res) => {
    ...
    try {
        await bigcommerce_1.default.delete(`/v3/carts/${cartId}`);
    }
    catch (err) {
        // 404 means BC already removed it — that's fine
        if (err.response?.status !== 404) {
            logger_1.default.error(`cart delete failed (cartId=${cartId}): ${err.message}`);
