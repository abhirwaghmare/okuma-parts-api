...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
            console.log('ProductDetails constructor called');
        super($scope, context);
    ...
    addProductToCart(event, form) {
        ...
        const waitMessage = $addToCartBtn.data('waitMessage') || 'Adding...';
        $addToCartBtn
            .val(waitMessage)
            .prop('disabled', true);
        ...
        // Gather product_id and quantity from form
        const formData = new FormData(form);
        const productId = formData.get('product_id');
        const quantity = formData.get('qty[]') || 1;

        // Build payload for external endpoint
        const payload = {
            line_items: [
                {
        ...
        // Call external endpoint for validation only
        console.log('Calling validation endpoint', payload);
        fetch('https://big-c-sdk.vercel.app/api/create-cart', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'x-call-source': 'storefront'
            },
        ...
        .then(async res => {
            ...
            if (validation && validation.valid) {
                // Proceed with original add-to-cart logic (storefront create cart)
                ...
                $addToCartBtn
                    ...
                    .prop('disabled', false);
                this.$overlay.hide();
                let errorMsg = (validation && validation.error) ? validation.error : 'Validation failed.';
                if (errorMsg.includes('Out of stock')) {
                    errorMsg = 'Sorry, this product is out of stock.';
                } else if (errorMsg.includes('Quantity per product must be less than 5')) {
                    errorMsg = 'You cannot add more than 5 units of this product.';
                }
                const tmp = document.createElement('DIV');
